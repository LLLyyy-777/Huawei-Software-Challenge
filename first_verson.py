...
def find_max_volume(index_volume, s_id_):
    """a = index_volume
    if (a[0][0] >= used_servers_volume[max_volume[0]][0][0] and a[0][0] >= used_servers_volume[max_volume[0]][1][0]) \
            or (a[1][0] >= used_servers_volume[max_volume[0]][0][0] and
                a[1][0] >= used_servers_volume[max_volume[0]][1][0]):
        max_volume[0] = s_id_
    if (a[0][1] >= used_servers_volume[max_volume[1]][0][1] and a[0][1] >= used_servers_volume[max_volume[1]][1][1]) \
            or (a[1][1] >= used_servers_volume[max_volume[1]][0][1] and
                a[1][1] >= used_servers_volume[max_volume[1]][1][1]):
        max_volume[1] = s_id_"""
...
def chose_server(v_):
    if max_volume:
        for i in range(len(max_volume)):  # 先循环已有的服务器
            ...
            s_.index = used_servers_volume[s_id_]
            if s_.judge1(v_) or s_.judge2(v_):
                type_ = 'old'
                return x_, s_, s_id_, type_
    for i in range(len(can_use_servers)):  # 循环需要购置的服务器
        x_ = can_use_servers[i]  # 机型
...
if __name__ == '__main__':
    t0 = time.time()
    training_path = r'D:\YAN\上海交大\华为软件挑战\training-data\test.txt'
    ...
    day_demand_nub = digital_nub[3:len(digital_nub)+1]  # 每天的需求数量
    ...
    for j in range(1, len(total_demands)+1):
        ...
        while demands[0][0] == 'del':
            info = server_vms[int(demands[0][1])]  # 该编号的虚拟机所放置的服务器位置信息
            vms_name = id_vms[int(demands[0][1])]  # 该编号的虚拟机名称
            v = vms[vms_name]  # 该虚拟机需要的容量
            if len(info) == 2:  # 单节点情况
                if info[1] == 'A':
                    loc = 0
                else:
                    loc = 1
                volume = used_servers_volume[info[0]]
                volume[loc][0] += int(v[0])
                volume[loc][1] += int(v[1])
                used_servers_volume[info[0]] = volume
                find_max_volume(volume, info[0])
            else:  # 双节点情况
                volume = used_servers_volume[info[0]]
                volume[0][0] += int(v[0]) / 2
                volume[0][1] += int(v[1]) / 2
                volume[1][0] += int(v[0]) / 2
                volume[1][1] += int(v[1]) / 2
                used_servers_volume[info[0]] = volume
                find_max_volume(volume, info[0])
            demands.pop(0)
        # 第一个需求
        ...
        for y in range(len(demands)):
            ...
            if d[0] == 'add':  # 添加虚拟机
                ...
                if y == len(demands)-1 or demands[y+1][0] == 'del':  # 最后一个或者倒数第二个后遇到del
                    ...
                    if tp == 'new':
                        day_used_servers.append([x, s_id])
                        total_used_servers.append([x, s_id])
            elif d[0] == 'del':  # 删除虚拟机
                info = server_vms[int(d[1])]  # 该编号的虚拟机所放置的服务器位置信息
                vms_name = id_vms[int(d[1])]  # 该编号的虚拟机名称
                v = vms[vms_name]  # 该虚拟机需要的容量
                if len(info) == 2:  # 单节点情况
                    if info[1] == 'A':
                        loc = 0
